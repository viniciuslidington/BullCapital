...
def read_prompt(filename: str) -> str:
    """Lê um prompt .md; se não existir, retorna um fallback seguro."""
    candidates = [
        PROMPTS_DIR / filename,                                    # /app/app/prompts/...
        Path(__file__).resolve().parents[2] / "prompts" / filename, # /app/prompts/...
        Path.cwd() / "app" / "prompts" / filename,                 # cwd/app/prompts
        Path.cwd() / "prompts" / filename,                         # cwd/prompts
    ]
    for p in candidates:
        if p.exists():
            return p.read_text(encoding="utf-8")

    # Fallbacks mínimos (evita crash do container)
    fallbacks = {
        "valuation_prompt.md": (
            "Você é um assistente de valuation. Explique os 6 métodos (DCF, Gordon, "
            "EV/EBIT, P/L, PEG, CAPM) de forma simples para iniciantes, incluindo "
            "o que o número significa na prática e limitações. Evite jargões."
        ),
        "multiplos_prompt.md": (
            "Você é um assistente de múltiplos. Explique P/L, P/VP, EV/EBITDA e "
            "EV/Receita em linguagem simples para iniciantes, como interpretar "
            "cada valor e as principais armadilhas."
        ),
    }
...
class FinancialAgent:
    ...
    def _setup_agents(self):
        ...
        self.ag_web = Agent(
            ...
            role="Lidar com buscas e pesquisas gerais na internet",
            model=OpenAIChat(id="gpt-4.1"),
            tools=[DuckDuckGoTools()],
            instructions="Sempre inclua as fontes das informações.",
    ...
    def _setup_team(self):
        ...
        self.equipe = Team(
            ...
            mode="coordinate",
            model=OpenAIChat(id="gpt-4o"),
            members=[self.ag_rag, self.ag_val, self.ag_mult, self.ag_web, self.ag_api_json],
            tools=[ReasoningTools(add_instructions=True)],
            enable_agentic_context=False,
            instructions=[
                "Crie relatório final em seções: Introdução, Fundamentação, Valuation, Múltiplos, Conclusão.",
                "Se o usuário pedir notícias/atualidades, inclua a seção 'Notícias recentes (com fontes)' usando o Agente de Busca na Web.",
...
def consultar_pdf_fundamentalista(question: str) -> str:
    ...
    pdf_path = Path(__file__).resolve().parents[1] / "indicadores_fundamentalistas (1).pdf"
...
# ---- CLI multi-modo: python -m app.agent.financial_agent PETR4 [--tables|--capm|--news]
if __name__ == "__main__":
    ...
    if "--tables" in flags:
        ...
        q = (
            f"Liste 5 notícias relevantes e recentes sobre {tk} nos últimos 90 dias. "
            "Para cada item, inclua: data, título, resumo em 1-2 linhas, fonte e URL. "
            "Responda em markdown e SEMPRE inclua as fontes."
        )
        print(agent._extract_response_content(agent.ag_web.run(q)))
    else:
