...
### 📁 .env específico do AI Service

Além do `.env` na raiz, o contêiner da IA copia um arquivo `.env` localizado em `backend/ai-service/.env`. Esse arquivo pode (em desenvolvimento) ser igual ao da raiz ou conter apenas as chaves relevantes ao serviço de IA. Mínimo recomendado:
...
Se optar por NÃO manter dois arquivos distintos, você pode copiar o `.env` raiz para dentro de `backend/ai-service/` antes do build. Certifique-se apenas de que nenhum segredo desnecessário seja incluído em imagens destinadas a produção.
